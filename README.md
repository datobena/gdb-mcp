...
- `start_binary(binary_path, args=None, cwd=None, load_init=True)` – launch gdb
  ...
  it so helpers like GEF/pwndbg remain enabled).
- `attach_to_pid(pid, cwd=None, load_init=True)` – start gdb and attach to a
  running process (same `load_init` behavior as above).
